...
<html>
<head>
    <title>View Scheduled Transfer - Banking System</title>
...
<%
    ...
    ScheduledTransfer transfer = (ScheduledTransfer) request.getAttribute("scheduledTransfer");
    if (transfer != null) {
        try {
            ...
            <div class="card card-banking">
                <div class="card-header bg-white">
                    <h5 class="card-title mb-0">
                        <i class="fas fa-eye me-2"></i>Scheduled Transfer Details
                    </h5>
                </div>
                <div class="card-body">
                    <c:choose>
                        <c:when test="${not empty scheduledTransfer}">
                            <table class="table table-bordered">
                                <tr><th>ID</th><td>${scheduledTransfer.id}</td></tr>
                                <tr><th>From Account</th><td><%= fromAccountNumber %></td></tr>
                                <tr><th>To Account</th><td><%= toAccountNumber %></td></tr>
                                <tr><th>Amount</th><td>Rs.${scheduledTransfer.amount}</td></tr>
                                <tr><th>Scheduled Date</th><td>${scheduledTransfer.scheduledDate}</td></tr>
                                <tr><th>Executed Date</th><td><c:out value="${scheduledTransfer.executedDate != null ? scheduledTransfer.executedDate : '-'}"/></td></tr>
                                <tr><th>Status</th><td>${scheduledTransfer.status}</td></tr>
                                <tr><th>Created By</th><td><c:out value="${scheduledTransfer.createdBy != null ? scheduledTransfer.createdBy.name : '-'}"/></td></tr>
                            </table>
                        </c:when>
                        <c:otherwise>
                            <div class="alert alert-danger mt-4">
                                <strong>Scheduled transfer not found.</strong><br>
                                The transfer you are looking for does not exist, may have been cancelled, or you do not have permission to view it.<br>
                                <span class="text-muted small">(If you believe this is an error, please contact support or your system administrator.)</span>
                            </div>
                        </c:otherwise>
                    </c:choose>
                    <a href="scheduled_transfers.jsp" class="btn btn-secondary mt-3"><i class="fas fa-arrow-left me-2"></i>Back to List</a>
                </div>
            </div>
        </div>
    </div>
